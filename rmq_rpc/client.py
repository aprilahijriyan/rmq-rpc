from aio_pika.abc import AbstractRobustChannel, AbstractRobustExchange, AbstractIncomingMessage, AbstractRobustQueue, ConsumerTag
import asyncio
...
class Client:
    def __init__(self, channel: AbstractRobustChannel, exchange: AbstractRobustExchange) -> None:
    ...
    @classmethod
    async def create(cls, channel: AbstractRobustChannel, exchange: AbstractRobustExchange):
        ...
        tmp_consumer = await tmp_queue.consume(instance.on_message_received, no_ack=True)
    ...
    async def on_message_received(self, msg: AbstractIncomingMessage):
        ...
        try:
            ...
            if message is None:
                raise TypeError(f"Message are not supported. Serializer is not available for {msg.content_type!r}")
    ...
    async def call(self, func: Union[Callable, str], args: List[Any] = [], kwargs: Dict[str, Any] = {}, **msg_kwargs):
        if callable(func):
            func = func.__name__

        if not isinstance(func, str):
            raise ValueError("func parameter must be a string. (this is the same as the function name)")
